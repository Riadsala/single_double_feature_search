...
get_cor_matrix <- function(df, key) {
  ...
  df %>% 
    ...
    select(-observer) %>%
    correlate() %>%
    mutate(.draw = key$.draw,
           feature = key$feature)-> dout
...
Now for each feature x sample, compute the correlations

```{r}

samples %>% 
 group_by(.draw, feature) %>%
  group_map(get_cor_matrix) %>%
  bind_rows -> dc

dc %>% mutate(block = parse_number(term)) %>%
  select(.draw, starts_with("block"), feature) %>%
  pivot_longer(-c(.draw, block, feature), names_to = "block2", values_to = "r") %>%
  group_by(feature, block, block2) %>%
  median_hdci(r, .width = c(.53, .97)) %>%
  filter(block2 == "block1") %>%
  ggplot(aes(block, ymin = .lower, y = r, ymax = .upper, fill = feature, group = interation(feature, .width))) + 
